...
description: Интеграция с Dereu — WhatsApp CPaaS-хабом поверх Meta Cloud API. Используй эту скилл, когда нужно провижинить компанию/тенант в Dereu, подключить WhatsApp-номер клиента через Embedded Signup (Model B), принять и обработать вебхук-форвард от Dereu (входящие сообщения, статусы доставки), отправить исходящее сообщение/медиа/шаблон/OTP, пометить входящее прочитанным / показать typing-индикатор, или депровижинить компанию. Триггеры — "Dereu", "подключить WhatsApp", "WABA", "Embedded Signup", "platform key", "webhook от Dereu", "dereu_company_id", "typing indicator", "прочитано", "mark as read".
---
...
Платит Meta не Dereu: у Dereu как Tech Provider нет кредитной линии, счёт за сообщения выставляется
бизнес-портфолио клиента (для этого проекта — собственному портфолио sala), и способ оплаты заводится
в его Meta Billing Hub. Без него шаблоны падают с `131042`, а с 2026-10-01 перестают доставляться и
сессионные сообщения (как минимум сверх бесплатных 1 000 в месяц на номер).
...
Строго: 1 организация партнёра = 1 company в Dereu = 1 номер WhatsApp. Физический номер в каждый момент
принадлежит ровно одной компании: любой проход Embedded Signup на уже занятый номер — перенос номера к
новой компании (см. `transferred` в §2).
...
- `transferred` присутствует всегда: `false` — только первый connect или реконнект той же компании;
  `true` — номер до этого принадлежал ЛЮБОЙ другой компании и переехал к вам — перенос номера
  (owner-consented move): проход Embedded Signup владельцем = его явное согласие, подтверждённое Meta.
  Переносом считается и повторный ES под другой компанией вашего же партнёра (например, другой
  организацией этого проекта). Идентификаторы прежнего владельца не раскрываются.
- `409` — `external_id` чужой, либо номер занят другой компанией при подключении через `system_user_token`
  (token-import не несёт ES-согласия владельца; не ретраить тем же запросом). ES-путь (`code`) на занятый
  номер — чьей бы компанией он ни был занят, вашего же партнёра или чужого, — НЕ 409, а перенос номера
  (см. `transferred` выше): вы получаете 201 с `transferred: true`, а прежняя компания-владелец
  отключается от номера (отправка и приём входящих для неё прекращаются сразу) и получает вебхук
  `waba_disconnected` (§3) — без идентификаторов нового владельца. Согласие при переносе подтверждается
  фактом доступа (fail-closed, в т.ч. для переноса внутри одного партнёра): токен свежего ES-обмена обязан
...
номера от любой прежней компании-владельца, в т.ч. другой компании вашего же партнёра (owner-consented
move, см. §2). Успешный ES на занятый номер на hosted-пути — всегда перенос (он идёт через ES владельца),
но `/connect/finish` может вернуть `409` с
...
- `business_app_message_echo` — сообщение, отправленное живым человеком из мобильного приложения
  ...
  коррелируются по идентификатору вашей отправки, которого здесь не было), и стоимости у него нет:
  по документации Meta о coexistence сообщения из приложения WhatsApp Business бесплатны и не влияют
  на тарификацию и окна Cloud API (в бесплатную квоту сервисных сообщений тоже не засчитываются) —
  стоимость на своей стороне не считайте. Дедуп — по `message_echoes[].id`. Приходят и
...
- `waba_disconnected` — владелец номера перенёс его на другую компанию (перенос номера, owner-consented
  move — см. §2) — чужого партнёра или другую компанию вашего же проекта (same-partner перенос): после
  этого события отправка с номера и приём входящих для этой `company_id` прекращены — прекратите отправку
  под отключённой `company_id` на своей стороне.
  ...
  нового владельца в событии нет — вы узнаёте только факт отключения (при same-partner переносе новая
  компания и так ваша — обе стороны переноса видны вам самим). Единственный `reason` сегодня —
  `owner_transfer`, в т.ч. при переносе внутри одного партнёра. Дедуп — по `event_id` (уникален на доставку).
...
## 4b. Прочитано + «печатает…» (mark-as-read / typing)

Перед ответом на входящее пометьте его прочитанным и покажите «печатает…» (UX живого оператора):
...
- Авторизация — **platform key** (не company api_key). `wamid` — из вебхука `message_received`.
- Body опционален: без него — только пометка прочитанным.
- `typing_indicator: true` = «печатает…» + прочитано (typing без прочтения у Meta не бывает);
  индикатор гаснет через ~25 сек или при отправке ответа — показывайте, только если реально ответите.
- Прочтение каскадное (более ранние сообщения тоже), допустимо 30 дней с момента получения.
- Ответ **200**: `{ "success": true }`. Ошибки: `404` `message_not_found`; `409` чужая компания /
  нет WABA; `422` Meta отклонила (напр. `131009` — невалидный wamid); `502` Meta недоступна
  (Dereu не ретраит — повторите сами, для read это безопасно).
...
- `dereu_company_id` ≠ `external_id`: `external_id` — ваш внутренний id, используется в путях
  ...
  `company_id` вебхука.
- Статусные форварды (`message_sent`/`message_delivered`/`message_read`) сейчас приходят без объекта
  Meta `statuses[].pricing` (`billable`, `pricing_model`, `type`: `regular` / `free_customer_service`,
  `category`) — Dereu его отбрасывает. Фактической тарификации и расхода бесплатной квоты проект не
  видит; стоимость оценивается по прайс-листу, а квота считается по собственному журналу.

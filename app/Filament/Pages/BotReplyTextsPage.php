 ...
 * тексты, которые бот отправляет сам, а не по тексту блока сценария —
 * нераспознанное нажатие кнопки, устаревшая кнопка, вопрос завершённого
 * запуска и ответ на вопрос про сервис, заданный на шаге «Запрос ввода
 * (AI)». Пустое поле означает стандартный текст из BotReplyKey.
 *
...
class BotReplyTextsPage extends Page
{
    ...
    public function form(Schema $schema): Schema
    {
        return $schema
            ->components([
                Form::make([
                    Text::make('Эти тексты бот отправляет сам, а не по тексту блока сценария: когда WhatsApp не смог передать нажатие кнопки, кнопка устарела, вопрос уже решён или человек на шаге «Запрос ввода (AI)» спросил про сам сервис. Пустое поле — используется стандартный текст.'),
                    ...array_map(

...
```bash
docker exec -e XDG_CONFIG_HOME=/tmp your-project-app-1 php artisan tinker --execute='...'
```

`your-project-app-1` runs the main checkout's code and database: in a git worktree use `make artisan` or `make shell` instead.
...
- `docker exec your-project-app-1 ...` from a git worktree — it runs the main checkout's code (Pint formats the main checkout's files); use `make pint`, `make artisan`, `make shell`.

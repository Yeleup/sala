...
test('автокомплит локаций отдаёт совпадения по началу названия', function () {
    ...
    // Название и цепочка родителей — отдельными полями: дропдаун рисует
    // их двумя строками (имя жирным, цепочка серым).
    $this->getJson('/locations/search?q=кара')
        ->assertOk()
        ->assertJsonPath('0.name', 'Каратауский район')
        ->assertJsonPath('0.chain', 'г.Шымкент');
});

test('пустой запрос автокомплита — вершина справочника, города первыми', function () {
    importSampleKatoTree();

    // Меню пустого поля: города республиканского значения, затем области.
    $this->getJson('/locations/search?q=')
        ->assertOk()
        ->assertJsonCount(2)
        ->assertJsonPath('0.name', 'г.Шымкент')
        ->assertJsonPath('0.chain', '')
        ->assertJsonPath('1.name', 'область Абай')
        ->assertJsonPath('1.chain', '');
});
...
test('автокомплит показывает найденный узел вместе с его районами', function () {
    ...
    $this->getJson('/locations/search?q=шымкент')
        ...
        ->assertJsonPath('0.name', 'г.Шымкент')
        ->assertJsonPath('0.chain', '')
        ->assertJsonPath('1.name', 'Абайский район')
        ->assertJsonPath('1.chain', 'г.Шымкент')
        ->assertJsonPath('2.name', 'Каратауский район')
        ->assertJsonPath('2.chain', 'г.Шымкент');
});
...
test('несколько слов в автокомплите сужают ветку', function () {
    ...
    $this->getJson('/locations/search?q=шымкент абай')
        ->assertOk()
        ->assertJsonCount(1)
        ->assertJsonPath('0.name', 'Абайский район')
        ->assertJsonPath('0.chain', 'г.Шымкент');
});

test('автокомплит предлагает исправление опечатки без разворота поддерева', function () {
    $district = locationNamed('Сарыагашский район', locationNamed('Туркестанская область'));
    $city = locationNamed('г.Сарыагаш', $district);
    locationNamed('мкр Достык', $city);

    $response = $this->getJson('/locations/search?q=сарагаш')->assertOk();
    $names = collect($response->json())->pluck('name');

    // «сарагаш» не начинает ни одного названия — предлагаются близкие
    // исправления; поддерево исправления не разворачивается (коррекция —
    // догадка, районы угаданного города похоронили бы конкурентов).
    expect($names)->toContain('г.Сарыагаш')
        ->and($names)->not->toContain('мкр Достык');
});

test('префиксные совпадения автокомплита не разбавляются нечёткими', function () {
    $district = locationNamed('Сарыагашский район', locationNamed('Туркестанская область'));
    locationNamed('г.Сарыагаш', $district);
    locationNamed('с.Карагаш', locationNamed('Карагашский с.о.', $district));

    $names = collect($this->getJson('/locations/search?q=карагаш')->assertOk()->json())->pluck('name');

    // «карагаш» начинает реальные названия — похожий «Сарыагаш» не
    // подмешивается: префикс достоверен, исправление здесь не нужно.
    expect($names)->toContain('с.Карагаш')
        ->and($names)->not->toContain('г.Сарыагаш');
});

test('короткий бессмысленный запрос автокомплита не корректируется', function () {
    importSampleKatoTree();

    // Ключ короче порога нечёткости: трограммная близость на коротких
    // словах — шум, а не исправление.
    $this->getJson('/locations/search?q=йцук')
        ->assertOk()
        ->assertExactJson([]);
});
